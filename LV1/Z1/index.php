...
for ($i = 1; $i <= 6; $i++) {
    ...
    foreach ($radovi as $radData) {
        ...
        $info = $rad->read();
        echo "<h3>" . htmlspecialchars($info['naziv_rada'] ?? '') . "</h3>";
        echo "<p>Link: <a href='" . htmlspecialchars($info['link_rada'] ?? '') . "'>" . htmlspecialchars($info['link_rada'] ?? '') . "</a></p>";
        echo "<p>OIB tvrtke: " . htmlspecialchars($info['oib_tvrtke'] ?? '') . "</p>";
        echo "<p>Tekst rada: " . nl2br(htmlspecialchars($info['tekst_rada'] ?? '')) . "</p>";
        echo "<hr>";
    }
}
